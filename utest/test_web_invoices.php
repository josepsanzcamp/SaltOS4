...
final class test_web_invoices extends TestCase
{
    ...
    public function test_insert(array $json): array
    {
        ...
        $json2 = test_web_helper('app/invoices/insert', [], $json['token'], '');
        ...
        $json2 = test_web_helper('app/invoices/insert', [
            'customer_name' => 'The SaltOS project',
            'customer_address' => 'X',
            'customer_country' => 'Y',
            'customer_city' => 'Barcelona',
            'customer_zip' => '08001',
            'customer_code' => '12345678X',
            'lines' => [
                [
                    'description' => 'ABC',
                    'quantity' => '1',
        ...
        $json3 = test_web_helper('app/invoices/insert', [
            'customer_name' => 'The SaltOS project',
            'lines' => [
                [
                    'nada' => 'nada',
    ...
    public function test_list(array $json): array
    {
        ...
        $json2 = test_web_helper('app/invoices/nada', [], '', '');
        $this->assertArrayHasKey('error', $json2);
    ...
    public function test_update(array $json): array
    {
        ...
        $json2 = test_web_helper('app/invoices/update', [], '', '');
        $this->assertArrayHasKey('error', $json2);

        $json2 = test_web_helper('app/dashboard/update', [], $json['token'], '');
        $this->assertArrayHasKey('error', $json2);

        $json2 = test_web_helper("app/invoices/update/$id", [], $json['token'], '');
        $this->assertArrayHasKey('status', $json2);
        $this->assertSame($json2['status'], 'ko');

        $json2 = test_web_helper("app/invoices/update/$id", [
            'nada' => 'nada',
        ], $json['token'], '');
        $this->assertArrayHasKey('status', $json2);
        $this->assertSame($json2['status'], 'ko');

        $details_ids = execute_query("SELECT id FROM app_invoices_lines WHERE invoice_id=$id");

        $json2 = test_web_helper("app/invoices/update/$id", [
            'customer_name' => 'The SaltOS project v2',
            'customer_code' => '12345678Z',
            'lines' => [
                [
                    'description' => 'GHI',
                    'quantity' => '7',
        ...
        $json3 = test_web_helper("app/invoices/update/$id", [
            'lines' => [
                [
                    'id' => 0,
                ],
    ...
    public function test_delete(array $json): void
    {
        ...
        $json2 = test_web_helper('app/invoices/delete', [], '', '');
        $this->assertArrayHasKey('error', $json2);

        $json2 = test_web_helper('app/dashboard/delete', [], $json['token'], '');
        $this->assertArrayHasKey('error', $json2);

        $json2 = test_web_helper("app/invoices/delete/$id", '', $json['token'], '');
        $this->assertSame($json2['status'], 'ok');
        $this->assertSame(count($json2), 2);
