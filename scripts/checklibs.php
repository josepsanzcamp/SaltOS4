...
function grep($pattern,$data) {
	...
	$result=array();
	foreach($data as $d) {
		if(stripos($d,$pattern)!==false) $result[]=$d;
	}
	$result=implode("\n",$result);
	return $result;
}
...
foreach($libs as $key=>$lib) {
	...
	if(count($lib)==4 && $lib[0][0]!="#" && (count($argv)==0 || in_array($lib[0],$argv))) {
		...
		$temp=head(1,$temp);
		if(substr($lib[3],0,7)!="base64:") $temp2=grep($lib[3],$temp);
		else $temp2=grep(base64_decode(substr($lib[3],7)),$temp);
		$isko=($temp2=="")?1:0;
		if($iserror) {
