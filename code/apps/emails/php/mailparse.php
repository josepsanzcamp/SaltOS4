 ...
 *
 * Layout: the actual polyfill logic lives in the __mailparse_*_helper()
 * functions/class below (never guarded: they can't collide with
 * anything the extension provides). Every mailparse_* function is just
 * a thin delegate to its same-named helper, grouped at the end of this
 * file and guarded with function_exists() so it's only defined when the
 * real extension is missing. That split exists so
 * utest/test_mailparse.php can call the helper directly and exercise
 * this polyfill's actual code even on machines that do have the native
 * extension installed, where the plain mailparse_* names are already
 * taken by the extension and can't be used to reach this file's code.
 */
...
/**
 * Thin mailparse_* delegates: only defined when the real extension is
 * missing, in which case each one just forwards to its same-named
 * __mailparse_*_helper() above.
 */
