...
saltos.app.__driver.type2.init = arg => {
    if (arg == 'list') {
        // This detect if the hash contains list and form at same time
        var hash = saltos.hash.get();
        hash = hash.split('/');
        if (hash.length >= 4) {
            var temp1 = hash.slice(0, 2).join('/');
            var temp2 = hash.slice(3).join('/');
            saltos.app.driver.open('#' + [temp1,temp2].join('/'));
        } else {
            saltos.app.driver.close();
        }
        // This reset the form fields to allow the next if
...
saltos.app.__driver.type2.open = arg => {
    var temp1 = saltos.hash.get().split('/').slice(0, 3);
    while (temp1.length <= 2) {
        temp1.push('');
    }
    temp1 = temp1.join('/');
    var temp2 = arg.split('/').slice(2, 4).join('/');
    saltos.hash.add([temp1,temp2].join('/'));
...
saltos.app.__driver.type2.close = arg => {
    ...
    // HASH PART
    var temp = saltos.hash.get().split('/').slice(0, 3).join('/');
