 ...
 * PDO PgSQL driver
 *
 * This file implements the PgSQL improved driver. This is the recommended driver when you want
 * to use PgSQL servers, it uses the PDO extension and alow to connect to PgSQL servers with a
 * good performance.
 */

/**
 * Database PDO PgSQL class
 *
 * This class allow to SaltOS to connect to PgSQL databases using the PDO driver
 */
class database_pdo_pgsql
{
    /**
    ...
    public function __construct($args)
    {
        ...
        $pass = $args['pass'] ?? '';
        try {
            $this->link = new PDO("pgsql:host=$host; port=$port; dbname=$name", $user, $pass);
        } catch (PDOException $e) {
            show_php_error(['dberror' => $e->getMessage()]);
        }
        $this->link->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $this->db_query("SET client_encoding TO 'UTF8'");
    }
